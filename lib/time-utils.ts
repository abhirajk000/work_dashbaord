...
export function normalizeReminderTimes(times: string[] | undefined): string[] {
  if (!times?.length) return [];
  return [...new Set(times.map((t) => normalizeTimeValue(t)).filter((t): t is string => Boolean(t)))].sort();
}
...
/** Human-readable 12h label for reminder chips and notification settings. */
